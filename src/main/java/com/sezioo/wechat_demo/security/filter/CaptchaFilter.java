...
public class CaptchaFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
       /* if (!StringUtils.equals("/authentication/form", request.getRequestURI()) ||
        ...
        String urlStr = securityProperties.getCode().getCaptcha().getUrl();
        ...
        if (matched && StringUtils.equalsIgnoreCase(HttpMethod.POST.name(), request.getMethod())) {
            ServletWebRequest webRequest = new ServletWebRequest(request);
            String captchaCode = webRequest.getParameter("captchaValidateCode");
            CaptchaValidateCode captchaValidateCode = (CaptchaValidateCode) request.getSession().getAttribute("captchaValidateCode");
            if (captchaValidateCode == null) {
                authenticationFailureHandler.onAuthenticationFailure(request, response, new ValidateAuthenticationException("验证码为空"));
                return;
            } else if (captchaValidateCode.getExpire().isBefore(LocalTime.now())) {
                authenticationFailureHandler.onAuthenticationFailure(request, response, new ValidateAuthenticationException("验证码过期"));
                return;
            } else if (!StringUtils.equalsIgnoreCase(captchaCode, captchaValidateCode.getCode())) {
                authenticationFailureHandler.onAuthenticationFailure(request, response, new ValidateAuthenticationException("验证码错误"));
                return;
            }
        }
